...
with h5py.File('dataset.h5', 'w') as hdf:
    ...
    for obj, path in enumerate(FOLDER_PATH, start=1):
        images = [file for file in os.listdir(path) if file.lower().endswith('.jpg')]
    ...
    hdf.create_dataset('Images', data=np.array(color_images), compression="gzip", compression_opts=9)
